...
EPOCHS = 300
BATCH_SIZE = 50 # Around 11 splits for full training dataset
LEARNING_RATE = 0.001
...
def load_data(file, max_sequence_length, relations):
    ...
    try:
        ...
        open(os.path.join(RESOURCE_DIR, file + ".vectors.npy")) as vec :
        ...
        with open(os.path.join(RESOURCE_DIR, file)) as fp:
            ...
            for i in dataset:
                ...
                try:
                    negative_paths = np.random.choice(negative_paths,NEGATIVE_SAMPLES)
                except ValueError:
                    if len(negative_paths) == 0:
                        negative_paths = neg_paths[-1]
                        print "Using previous question's paths for this since no neg paths for this question."
                    index = np.random.randint(0, len(negative_paths), NEGATIVE_SAMPLES)
            ...
            open(os.path.join(RESOURCE_DIR, file + ".vectors.npy"), "w") as vec:
...
def main():
    ...
    max_length = 25
    relations = load_relation(os.path.join(RESOURCE_DIR,'relations.pickle'))
    vectors, questions, pos_paths, neg_paths = load_data("tutorial_id_data_set.json", max_length, relations)
